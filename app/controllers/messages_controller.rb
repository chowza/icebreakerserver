class MessagesController < ApplicationController
	...
	def show
		...
		@match_time = Match.where("swipee_id = ? AND profile_id = ? OR swipee_id = ? AND profile_id = ?", @profile['id'],params[:recipient_id],params[:recipient_id],@profile['id']).limit(1).pluck(:match_time)[0]
	...
	def create
		...
		@match_time = Match.where("swipee_id = ? AND profile_id = ? OR swipee_id = ? AND profile_id = ?", @message.profile_id,@message.recipient_id,@message.recipient_id,@message.profile_id).limit(1).pluck(:match_time)[0]
