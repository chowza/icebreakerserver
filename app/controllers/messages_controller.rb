class MessagesController < ApplicationController
	...
	def show
		...
		@match_time = Match.where("profile_id = ? AND swipee_id = ?", @profile['id'],params[:recipient_id]).limit(1).pluck(:match_time)
	...
	def create
		...
		@match_time = Match.where("swipee_id = ? AND profile_id = ?", @message.profile_id,@message.recipient_id).pluck(:match_time)
